...
def parse_datetime_with_offset(value):
    """Парсить yyyy-mm-dd hh:mm:ss+HH:MM і повертає aware datetime."""
    try:
        # Розділяємо datetime та offset
        dt_part = value[:-6]
        offset_part = value[-6:]

        dt = datetime.strptime(dt_part, "%Y-%m-%d %H:%M:%S")

        # Парсимо GMT-зміщення
...
with open(INPUT_FILE, encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        # install_time / event_time (повний формат)
        ...
        if row["is_churned"] == "NA":
            ...
            # Створюємо структуру з об'єднаним datetime
...
if invalid_rows:
    # для invalid беремо row + додаємо errors як текст
...
for item in invalid_rows:
    ...
    # Конвертуємо dict → "key=value" і об'єднуємо

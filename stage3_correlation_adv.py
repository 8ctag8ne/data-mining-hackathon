...
feature_cols = [
    ...
    if col not in ["user_id", "is_advanced"]
]
...
def compute_advance_bins(df, col):
    ...
    grouped = df.groupby("_bin")["is_advanced"].mean()
...
for col in feature_cols:
    ...
    # Кореляція
    try:
        corr = df["is_advanced"].corr(data)
    except Exception:
        corr = None

    # Churn rate у бінів
    bins = compute_advance_bins(df, col)

    if bins is not None:
        for interval, advance_rate in bins.items():
            results.append({
                "feature": col,
                "bin": str(interval),
...
out.to_csv("advance_dependency.csv", index=False)

print("\nГотово! Збережено advance_dependency.csv")
...
for feature in out["feature"].unique():
    ...
    plt.title(f"Correlation with advance: {feature}\n({corr_value:.3f})")
    plt.xlabel("Correlation")
    plt.ylabel("")
    plt.tight_layout()
    plt.savefig(f"feature_charts_adv/{feature}_correlation.png")
    plt.close()
    ...
    sns.barplot(data=subset, x="bin", y="advance_rate")
    plt.xticks(rotation=45)
    plt.title(f"Advance rate across bins: {feature}")
    plt.tight_layout()
    plt.savefig(f"feature_charts_adv/{feature}_bins.png")
    plt.close()

print("\nГрафіки збережено у папку feature_charts_adv/")
